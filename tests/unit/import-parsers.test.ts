...
describe("deteccao de formato", () => {
  it("reconhece cada formato pelo conteudo, nao pela extensao", () => {
  ...
  it("orienta quem tenta subir .xls antigo", () => {
...
describe("encoding", () => {
  it("decodifica latin-1 mesmo quando o header do OFX diz USASCII", () => {
  ...
  it("preserva acentos de arquivo latin-1", () => {
...
describe("OFX 1.x (SGML)", () => {
  ...
  it("extrai todos os lancamentos", () => {
    expect(result.fileType).toBe("OFX");
    expect(result.transactions).toHaveLength(5);
  });

  it("nao confunde o saldo final com lancamento", () => {
  ...
  it("le data, valor e sinal corretamente", () => {
  ...
  it("junta NAME e MEMO quando se complementam", () => {
    const complementar = parseStatement(
      Buffer.from(
        [
  ...
  it("identifica a conta declarada no arquivo", () => {
...
describe("CSV com ponto e virgula", () => {
  ...
  it("descarta cabecalho do banco, saldo anterior e rodape", () => {
  ...
  it("entende sinal a direita e parenteses como negativo", () => {
    const aluguel = result.transactions.find((t) => t.description.includes("ALUGUEL"));
  ...
  it("nao confunde a coluna Saldo com a coluna Valor", () => {
...
describe("CSV com virgula e colunas de credito/debito", () => {
  const result = parseStatement(fixture("extrato-csv-virgula.csv"), "extrato.csv");

  it("detecta o separador mesmo com virgula dentro da descricao", () => {
    expect(result.separator).toBe(",");
  });

  it("le descricao entre aspas contendo o separador", () => {
  ...
  it("transforma credito em entrada e debito em saida", () => {
    const venda = result.transactions.find((t) => t.description.includes("VENDA"));
  ...
  it("entende ano de dois digitos", () => {
    expect(result.transactions[0].date.toISOString().slice(0, 10)).toBe("2026-08-02");
  });
});

describe("XLSX", () => {
  it("usa a primeira aba por padrao e lista as demais", () => {
    const result = parseStatement(fixture("extrato.xlsx"), "extrato.xlsx");
  ...
  it("ignora titulo antes da tabela e total no rodape", () => {
    const result = parseStatement(fixture("extrato.xlsx"), "extrato.xlsx", {
      sheetName: "Lancamentos",
    });
...
describe("limites da Secao 5.6", () => {
  it("recusa arquivo acima de 10 MB", () => {
    ...
    expect(() => parseStatement(big, "grande.csv")).toThrow(/10 MB/);
  });

  it("recusa arquivo vazio", () => {
    expect(() => parseStatement(Buffer.alloc(0), "vazio.csv")).toThrow(ParseError);
  });

  it("recusa formato desconhecido", () => {
    expect(() => parseStatement(Buffer.from("nada aqui"), "arquivo.bin")).toThrow(
...
describe("datas", () => {
  ...
  it("le a data do OFX com hora e fuso", () => {
...
describe("separador do CSV", () => {
  it("escolhe ; quando a descricao tem virgulas", () => {
    const text = [
  ...
  it("escolhe , quando e esse o separador", () => {
...
describe("normalizacao e dedupe", () => {
  it("normaliza para comparacao sem perder o original", () => {
  ...
  it("gera o mesmo hash para descricoes equivalentes", () => {
    const base = {
      accountId: "conta-1",
  ...
  it("gera hash diferente quando algo muda de verdade", () => {
    const base = {
      accountId: "conta-1",
      date: new Date(Date.UTC(2026, 7, 20)),
      amountCents: -4990,
...
describe("reimportacao do mesmo arquivo", () => {
  it("produz exatamente os mesmos hashes (Secao 5.2)", () => {
    const first = parseStatement(
      fixture("extrato-csv-ponto-virgula.csv"),
      "extrato.csv",
    );
    const second = parseStatement(
      fixture("extrato-csv-ponto-virgula.csv"),
      "extrato.csv",
    );
